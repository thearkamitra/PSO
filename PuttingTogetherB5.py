        ...
        for i in range(len(Y)):
            ...
            for j in range(len(Z)):
                if "E(" in Z[j]:
                    X=Z[j+3]
                    file.close()
                    coordinates.append(position[int(name[3])])
                    energies.append(X)
...
def function(position,iternum,partnum):
        ...
        while normal_term(name[:-3]+"log")==0: 
            pass
        valuesnow[i]=energy(name,valuesnow[i])
...
for k in range(1,n):
    ...
    velocity= w*velocity-c1*r1*(position-best_individual)-c2*r2*(position-best_pos)
    ...
    print("--------------------------------")
    print (len(energies))
    f=open("all_value.txt",'w+')
    for i in range(len(energies)):
        f.write(str(strnames[i])+"\n")
